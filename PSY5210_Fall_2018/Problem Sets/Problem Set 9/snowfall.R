...
## This example is intended to cumulatively address many of the 
## things we have learned over the course of the semester.
...
##
## For the weather data set, look at thhe snowfall across months of the year.  You are
## a climatologist tasks to help residents better understand the the snowfall
## patterns in Houghton, both within a year and historically.  Your ultimate 
## product will be a short report meant for planners and historians.

##
## Create two graphics that show basic snowfall trends, one giving some type of summary
## over the entire data set from 1890, and  one showing the trends across the 
## different months of the year.  
## For each graphic, write a paragraph decription of the trends you are displaying.
##
## For each graph, you will get a baseline credit of 5 points for creating the 
## the graphic, 3 points for the text description, and up to 7 additional points
## (1 pt. each) for additional improvement to the graph.  These improvement could
## include adding color, gridlines, or otherwise changing the graphical appearance,
## adding layers showing means/medians/max/min or 
## other percentiles, adding a legend, etc.

library(gplots)
filter <- 1:128 ##ignor the last year, update this if you get more data:
...
plot(icebymonth[550:900],type="l")


## A straight matplot is sort of hard to grasp:
matplot(weather[,2:9],type="l")

barplot(as.matrix(weather[,2:9]))
cols <- rev(rich.colors(8,"blues"))
barplot(t(as.matrix(weather[,2:9])),col=cols)
...
##Question 2: 
## There appears to be a shift in snowfall patterns
## over the past century, with an increase happening around 1920.
## Consider two time eras, one for the 30 years before 1920, and one 
## for the 93 years after 1920.  Conduct 
## statistical tests at p=.05 that show whether snowfall increased in each month
## of the year, or just some of the months.
...
## than January, but this has switched.  Conduct an appropriate statistical
...
## than December? Describe the results as you would in a research report.  You will get
## 2 points for conducting a test that answers the question, 2 additional points 
## for conducting the most appropriate test for the question, and 2 points for
## properly describing the results.
...
##
##
##You can find out which month had the highest snowfall using code such as:
...
##Create a table showing how many times each month got the highest
## snowfall, for the early and late eras.  Format the table as if you were
## publishing the data in a brochure or magazine article.  Did the distribution
## change between the early and late era?  Conduct a statistical test, 
## (using a p value criterion of 0.10) to determine if this difference is
## just 'by chance'.  Write a paragraph describing your results.
## Points: 3 pts for creating the table, 3 pts for conducting the proper statistical test,
## 3 pts for describing the results.
...
year1617 <- data.frame(Year="2016-17",
